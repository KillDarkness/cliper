...
func main() {
	...
	if cfg.AutoVideoSize {
		log.Printf("capture resolution: %s (auto; override with CLIPER_VIDEO_SIZE=WIDTHxHEIGHT)", cfg.VideoSize)
	} else {
		log.Printf("capture resolution: %s (configured)", cfg.VideoSize)
	}
	log.Printf("capture fps: %d", cfg.FPS)
	log.Printf("target buffer: %s (%d x %s segments)", cfg.ClipDuration(), cfg.MaxSegments, cfg.SegmentDuration)
	log.Printf("encoder: %s preset=%s crf=%d pixel_format=%s", cfg.VideoCodec, cfg.EncoderPreset, cfg.CRF, cfg.PixelFormat)
