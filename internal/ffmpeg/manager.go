...
func (m *Manager) recordArgs() []string {
	...
	args = append(args,
		...
		"-preset", m.cfg.EncoderPreset,
		"-tune", "zerolatency",
		"-crf", fmt.Sprint(m.cfg.CRF),
		"-pix_fmt", m.cfg.PixelFormat,
		"-f", "segment",
...
func (m *Manager) inputArgs() []string {
	...
	case config.BackendPipeWire:
		return []string{"-f", "pipewire", "-framerate", fmt.Sprint(m.cfg.FPS), "-video_size", m.cfg.VideoSize, "-i", m.cfg.PipeWireNode}
	...
	default:
		return []string{
			"-f", "x11grab",
			"-draw_mouse", drawMouseValue(m.cfg.DrawMouse),
			"-framerate", fmt.Sprint(m.cfg.FPS),
			"-video_size", m.cfg.VideoSize,
			"-i", x11Input(m.cfg.Display, m.cfg.CaptureOffset),
		}
	}
}
...
func x11Input(display, offset string) string {
	if offset == "" || offset == "0,0" {
		return display
	}
	return fmt.Sprintf("%s+%s", display, offset)
}

func drawMouseValue(enabled bool) string {
	if enabled {
		return "1"
	}
	return "0"
